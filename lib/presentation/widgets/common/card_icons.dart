...
class _FlashcardsTabState extends State<FlashcardsTab> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      // 2. Получаем модули вместе со связанными карточками (для подсчета прогресса)
      ...
      final parsedModules = (modulesData as List<dynamic>).map((m) {
        ...
        return {
          ...
          'author': 'Вы', // Так как user_id совпадает с текущим пользователем
          'progress': progress,
          'total': total,
  ...
  Widget _buildContinueLearningCarousel() {
    // Берем модули, где есть карточки
        ...
        children: [
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: () {}, // Переход к прохождению карточек
          ...
          if (items.length > 1) Row(
            ...
            children: List.generate(items.length, (i) => Container(
              ...
              width: 24, height: 6,
              decoration: BoxDecoration(
                color: i == 0 ? AppColors.accent : const Color(0xFFD9D9D9),
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2))),
            const SizedBox(height: 24),
            _buildCreateOptionRow('Модуль', FlashcardIconPainter(color: AppColors.primary), () => Navigator.pop(ctx)),
            const SizedBox(height: 14),
